...
gfxQuad1:setTexture ( "images/background.png" )
...
gfxQuad2:setTexture ( "images/background2.png" )
...
tileLib:setTexture ( "images/player.png" )
tileLib:setSize ( 2,2 )
tileLib:setRect ( -1,-1,1,1)

tileLib2 = MOAITileDeck2D.new ()
tileLib2:setTexture ( "images/meter.png" )
...
function onCollideLaser ( event, fixtureA, fixtureB, arbiter )
...
function onCollide ( event, fixtureA, fixtureB, arbiter )
	...
	if event == MOAIBox2DArbiter.BEGIN then	
		...
		print( fixtureA.name.." collided with " .. fixtureB.name)
			
			if fixtureA.name and fixtureB.name then
		print( fixtureA.name.." collided with " .. fixtureB.name)
			
			if (fixtureA.name=="Laser") then 
				contactx,contacty=fixtureA:getBody():getPosition()
				print ("Contactx="..contactx.." ContactY="..contacty)
			end
			if (fixtureB.name=="Laser") then 
				contactx,contacty=fixtureB:getBody():getPosition()
				print ("Contactx="..contactx.." ContactY="..contacty)
				plot = MOAIGfxQuad2D.new ()
				plot:setTexture ( "images/cloud.png" )
				plot:setRect(  -0.25,-0.25,0.25,0.25)
				plot1 = MOAIProp2D.new ()
				plot1:setDeck ( plot )
				plot1:seekLoc ( contactx,contacty, 0)
				layer:insertProp ( plot1)
...
function clickCallback ( down )
	...
	if down==false and playerpicked then
		...
		launch_laser(forceX,forceY,bx,by)
...
function launch_laser(forceX,forceY,bx,by)
laser_body = world:addBody ( MOAIBox2DBody.DYNAMIC )
laser_ball = laser_body:addCircle ( 0,0,0.2)
laser_body:setTransform(bx,by,0)
laser_body:isBullet(true)
laser_body:setLinearVelocity( forceX *10000, forceY *10000)
laser_ball:setSensor()
laser_ball.name="Laser"
laser_body:resetMassData()
laser_ball:setCollisionHandler ( onCollide, MOAIBox2DArbiter.PRE_SOLVE + MOAIBox2DArbiter.BEGIN + MOAIBox2DArbiter.END, 0x00 )
...
wall_textures[3] :setTexture ( 'images/wall1.png' )
...
wall_textures[4] :setTexture ( 'images/wall1.png' )
...
wall_textures[1] :setTexture ( 'images/wall1.png' )
...
wall_textures[2] :setTexture ( 'images/ground.png' )
